  ...
  while True:
    ...
    for index, row in data.iterrows():
        ...
        if len(y_batch) == batch_size:
          ...
          x_batch = e.sents2elmo(my_sents, output_layer=-2)
          if np.shape(x_batch) != (batch_size,3,200,1024):
            x_batch = []
            y_batch = []
            continue
          x_batch = np.array(x_batch).reshape(batch_size,200,3072)
...
def declare_model():
  ## Input
  sample = Input(batch_shape=(10, 200, 3072))
...
history = m.fit_generator(generate_embeddings(10), steps_per_epoch=2400, epochs=10)
with open("elmo1_history","w") as file:
  file.write(str(history.history))


score = m.evaluate_generator(generate_embeddings_eval(10), steps=800)
print(score)
m.save("model_elmo1_local.h5")
