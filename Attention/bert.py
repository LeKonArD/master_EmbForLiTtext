  ...
  while True:
    ...
    for index, row in data.iterrows():
        ...
        if len(y_batch) == batch_size:
          ...
          if np.shape(x_batch) != (batch_size,200,3072):
            x_batch = []
            y_batch = []
            continue
...
def generate_sentences(batch_size):
  ...
  while True:
    ...
    data = pd.read_csv("testframe_final.tsv", sep="\t",error_bad_lines=False,  quotechar="µ")
...
def declare_model():
  ## Input
  sample = Input(batch_shape=(100, 200, 3072))
